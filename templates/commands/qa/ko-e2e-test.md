...
- **`pwHelper` methods take Locators, not selector strings.** Wrap at the call site: `await pwHelper.click(this.locator(this.elements.submitButton))`. Never call `page.*` directly. <!-- /ko-onboard: confirm the exact wrap helper name from BasePage/pwHelper -->
